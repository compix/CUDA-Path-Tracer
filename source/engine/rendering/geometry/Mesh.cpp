...
BBox Mesh::computeBBox(const SubMesh& subMesh)
{
	BBox box;
	for (auto& v : subMesh.vertices)
		box.unite(v);
...
void Mesh::scale(SubMesh& subMesh, const glm::vec3& s)
{
	for (auto& v : subMesh.vertices)
		v *= s;
}

void Mesh::mapToUnitCube(SubMesh& subMesh)
{
	BBox meshBBox = subMesh.computeBBox();
	float scaleInv = 1.0f / meshBBox.scale()[meshBBox.maxExtentIdx()];
	glm::vec3 offset = -meshBBox.min();

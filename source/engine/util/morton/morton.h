...
namespace morton
{
    ...
    inline HOST_AND_DEVICE uint64_t separateBy3(uint32_t n)
    {
    ...
    inline HOST_AND_DEVICE uint64_t compactBy3(uint64_t n)
    {
	...
	inline HOST_AND_DEVICE uint32_t compactBy3(uint32_t x)
	{
		x &= 0x09249249;             
		x = (x ^ (x >> 2)) & 0x030c30c3;
		x = (x ^ (x >> 4)) & 0x0300f00f;
    ...
    inline HOST_AND_DEVICE uint64_t encode63(uint32_t x, uint32_t y, uint32_t z)
    {
    ...
    inline HOST_AND_DEVICE void decode63(uint64_t m, uint32_t& x, uint32_t& y, uint32_t& z)
    {
	...
	inline HOST_AND_DEVICE uint32_t encode30(uint32_t x, uint32_t y, uint32_t z)
	{
		x = (x | (x << 16)) & 0x030000FF;
		x = (x | (x << 8)) & 0x0300F00F;
		x = (x | (x << 4)) & 0x030C30C3;
	...
	inline HOST_AND_DEVICE void decode30(uint32_t code, uint32_t& x, uint32_t& y, uint32_t& z)
    {
		x = compactBy3(code);
		y = compactBy3(code >> 1);
		z = compactBy3(code >> 2);
	...
	inline HOST_AND_DEVICE uint64_t encode60(uint32_t x, uint32_t y,uint32_t z)
	{
...
{
	HOST_AND_DEVICE MortonCode3D() {}
	HOST_AND_DEVICE MortonCode3D(uint32_t x, uint32_t y, uint32_t z) : value(morton::encode30(x, y, z)) {}
	HOST_AND_DEVICE ~MortonCode3D() {}

	HOST_AND_DEVICE operator uint32_t() const
	{
		return value;
	}

	HOST_AND_DEVICE void decode(uint32_t x, uint32_t y, uint32_t z) const
	{
...
{
	HOST_AND_DEVICE MortonCode3D() {}
	HOST_AND_DEVICE MortonCode3D(uint32_t x, uint32_t y, uint32_t z) : value(morton::encode63(x, y, z)) {}
	HOST_AND_DEVICE ~MortonCode3D() {}

	HOST_AND_DEVICE operator uint64_t() const
	{
		return value;
	}

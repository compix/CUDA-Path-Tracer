...
#define CUDA_STRING_STREAM 0

class Logger
{
public:
    using StreamKey = uint32_t;
    ...
    static StreamKey getStringStreamKey() { return m_curAvailableStrStreamKey++; }

    static bool hasStringStream(StreamKey key) { return m_stringStreams.count(key) > 0; }
    static std::stringstream& stringStream(StreamKey key) { return m_stringStreams[key]; }
private:
    static StreamKey m_curAvailableStrStreamKey;
    static std::unordered_map<StreamKey, std::stringstream> m_stringStreams;
};
